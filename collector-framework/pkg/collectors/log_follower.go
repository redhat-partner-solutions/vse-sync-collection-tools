...
func (logs *LogsCollector) poll() error {
	podName := logs.ctx.GetPodName()
	podLogOptions := v1.PodLogOptions{
		SinceTime:  &metav1.Time{Time: logs.lastPoll.Time()},
		Container:  logs.ctx.GetContainerName(),
		Follow:     true,
		Previous:   false,
		Timestamps: true,
	}
	podLogRequest := logs.ctx.GetClientset().K8sClient.CoreV1().
		Pods(logs.ctx.GetNamespace()).
...
func getArgsFromCollectorArgs(constructor *CollectionConstructor) (
	...
	logArgs, ok := constructor.CollectorArgs["Logs"]
	if !ok {
		return false, "", "", false, errors.New("no Logs args in collector args")
	}
	logsOutputFileRaw, ok := logArgs["logsOutputFile"]
	if !ok {
		return false, "", "", false, errors.New("no logsOutputFile in Log collector args")
	}

	logsOutputFile, ok = logsOutputFileRaw.(string)
	if !ok {
		return false, "", "", false, errors.New("logsOutputFile is not a string")
	}

	includeLogTimestampsRaw, ok := logArgs["includeLogTimestamps"]
	if !ok {
		return false, "", "", false, errors.New("no logsOutputFile in Log collector args")
	}

	includeLogTimestamps, ok = includeLogTimestampsRaw.(bool)
	if !ok {
		return false, "", "", false, errors.New("logsOutputFile is not a bool")
	}

	tempDirRaw, ok := logArgs["tempDir"]
	if !ok {
		return false, "", "", false, errors.New("no tempDir in Log collector args")
	}

	tempDir, ok = tempDirRaw.(string)
	if !ok {
		return false, "", "", false, errors.New("tempDir is not a string")
	}

	keepDebugFilesRaw, ok := logArgs["keepDebugFiles"]
	if !ok {
		return false, "", "", false, errors.New("no keepDebugFiles in Log collector args")
	}

	keepDebugFiles, ok = keepDebugFilesRaw.(bool)
	if !ok {
		return false, "", "", false, errors.New("keepDebugFiles is not a string")
	}

	return includeLogTimestamps, logsOutputFile, tempDir, keepDebugFiles, nil
}
...
func NewLogsCollector(constructor *CollectionConstructor, ctx *clients.ContainerExecContext) (Collector, error) {
	includeLogTimestamps, logsOutputFile, tempDir, keepDebugFiles, err := getArgsFromCollectorArgs(constructor)
	if err != nil {
		return &LogsCollector{}, err
	}

	collector := LogsCollector{
		APICollector: NewAPICollector(
			logPollInterval,
			false,
		...
		withTimeStamps:     includeLogTimestamps,
		logsOutputFileName: logsOutputFile,
		generations: loglines.Generations{
			Store:  make(map[uint32][]*loglines.LineSlice),
			Dumper: loglines.NewGenerationDumper(tempDir, keepDebugFiles),
		},
	}
