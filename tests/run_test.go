...
// This is a workaround to set the LogLevel to Debug before flags are parsed
// or the config is loaded. It is necessary because flags can only be parsed
// once globally which must be after ginkgo registers its flags, and because
// logging occurs when config sections are registered by Suite packages.
func setLogLevel() bool {
	log.SetLevel(log.DebugLevel)
	return true
}
var _ = setLogLevel()
// The workaround could be made unnecessary if the framework had control over
// when the test Suite packages are loaded. One possibility for this would be
// dynamically loading Suite packages as Go plugins but that method has other
